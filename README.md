...
2. **甲子園(トップのアプリ)**: `data/koshien/results.csv` を編集する
   ...
   - **都道府県予選の自動抽出**: `tools/fetch_chiba.py` + `tools/chiba_integrate.py` が千葉の例。
     URLは `bibijr.vivian.jp/<県ローマ字>/<選手権の回次>`(東西分割年は末尾 e/w)。
     他県も同じ要領でスクリプトを複製して対応可能。合同予選(南関東・東関東等)の年は大会名で自動除外
