...
(function () {
  ...
  function render(root, ti) {
    ...
    note.textContent = isFull
      ? "※ 全出場校のトーナメント(毎日自動更新)。校名の後ろの順位は現在(最新年時点)のランキング。"
    ...
    function renderFull() {
      ...
      T.teams.forEach(function (t, i) {
        var g = el("g", { "class": "bteam " + (t.an === "start" ? "bL" : "bR") }, svg);
        ...
        txt.textContent = t.n;
        var rs = el("tspan", { "font-size": 9.5, "font-weight": "normal", fill: SUB, "class": "bkt-rankfull" }, txt);
        var r0 = rankShort(t.n, String(B.defaultWindow));
        rs.textContent = r0 ? "　" + r0 : "";
        rs.__name = t.n;
      });
