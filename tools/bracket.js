...
(function () {
  ...
  function render(root, ti) {
    ...
    note.textContent = isFull
      ? "※ 全出場校のトーナメント(毎日自動更新)。順位は現在(最新年時点)のランキング、回数は当サイト収録データ(ベスト8以上)基準。"
    ...
    head.querySelector("#bkt-win").addEventListener("change", function () {
      ...
      svg.querySelectorAll(".bkt-rankfull").forEach(function (t) {
        ...
        t.textContent = r ? (t.__side === "R" ? r + "　" : "　" + r) : "";
      });
    });
    ...
    function renderFull() {
      ...
      T.teams.forEach(function (t, i) {
        ...
        if (t.app) {
          el("text", { x: t.x, y: t.y + 14.5, "text-anchor": t.an, "font-size": 9,
            fill: SUB, "font-family": "sans-serif" }, g).textContent = t.app;
        }
      });
